...
static const CGFloat CSToastVerticalPadding     = 00.0;
...
static BOOL isToasting                          = NO;       //the flag that view is Toasting
...
- (void)makeToastLong:(NSString *)message {
    [self makeToastLong:message duration:CSToastDefaultDuration position:CSToastDefaultPosition];
}

- (void)makeToastLong:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToastLong:toast duration:duration position:position];
}



- (void)showToastLong:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point {
    toast.center = [self centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    
    if (CSToastHidesOnTap) {
        UITapGestureRecognizer *recognizer = [[UITapGestureRecognizer alloc] initWithTarget:toast action:@selector(handleToastTappedLong:)];
    ...
    [UIView animateWithDuration:CSToastFadeDuration
                     ...
                     animations:^{
                         ...
                         NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(toastTimerDidFinishLong:) userInfo:toast repeats:NO];
...
- (void)toastTimerDidFinishLong:(NSTimer *)timer {
    [self hideToastLong:(UIView *)timer.userInfo];
}

- (void)handleToastTappedLong:(UITapGestureRecognizer *)recognizer {
    NSTimer *timer = (NSTimer *)objc_getAssociatedObject(self, &CSToastTimerKey);
    [timer invalidate];
    
    [self hideToastLong:recognizer.view];
}



- (void)hideToastLong:(UIView *)toast {
    [UIView animateWithDuration:CSToastFadeDuration
                          delay:1.0
...
//        CGSize expectedSizeMessage = [self sizeForString:message font:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        messageLabel.frame = CGRectMake(0.0, 0.0, 220, 88);
    }
...
//    CGFloat wrapperWidth = MAX((imageWidth + (CSToastHorizontalPadding * 2)), (longerLeft + longerWidth + CSToastHorizontalPadding));    
    ...
    wrapperView.frame = CGRectMake(0.0, 0.0, 220, wrapperHeight);
    
    messageWidth = wrapperView.frame.size.width - messageLeft * 2;
